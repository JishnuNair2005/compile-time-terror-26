...
export default function TeacherDashboard({ route, navigation }) {
  ...
  const renderSummary = () => {
    if (loadingSummary) return (
      <View style={styles.setupContainer}><ActivityIndicator size="large" color="#000066" /><Text style={{ marginTop: 20, color: '#64748B', fontWeight: 'bold' }}>Extracting AI Summary...</Text></View>
    );
    return (
      <ScrollView contentContainerStyle={styles.scrollCenter}>
        ...
        {finalSummary.length === 0 ? (
          <View style={{ marginTop: 30, padding: 24, backgroundColor: '#F8FAFC', borderRadius: 16, width: '90%' }}>
            <Text style={{ fontSize: 16, color: '#64748B', textAlign: 'center' }}>Raw data is safely stored. AI summary failed to load.</Text>
          </View>
        ) : (
